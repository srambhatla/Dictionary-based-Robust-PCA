...
function[done] = run_lr_dict_col_sp(lr, k, d, out_folder)
...
for i4 = 1:numel(d)
    for i3 = 1:numel(k)
        ...
        parfor i = 1:monte
            ...
            % Generate the data 
            [X(:,:,i), R(:,:,i), A(:,:,i)] = gen_dat_col(n, m, d_p, r_p, k_p);
            ...
            [X_est_m(:,:,i), A_est_m(:,:,i), ~, ~] = run_prox_grad_col(Y, R(:,:,i), X_init, A_init, X(:,:,i), A(:,:,i)); 
        end
